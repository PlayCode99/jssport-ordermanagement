...
export default function EmbroideryTeamsPage({ rows: initialRows }: PageProps) {
    ...
    const saveForm = async () => {
        ...
        const endpoint =
            editId === null
                ? '/settings/data/embroidery-teams'
                : `/settings/data/embroidery-teams/${editId}`;
    ...
    const toggleActive = async (row: EmbroideryTeamRow) => {
        const response = await fetch(
            `/settings/data/embroidery-teams/${row.id}`,
            {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    ...
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                        <div>
                            <p className="text-xs font-semibold tracking-[0.12em] text-slate-500 uppercase">
                                Embroidery Team Master
                            </p>
                            <h1 className="mt-2 text-2xl font-semibold text-slate-900">
                                จัดการทีมห้องปัก
                            </h1>
                            <p className="mt-2 text-sm text-slate-600">
                                กำหนดทีมที่ใช้งานจริงในหน้าแจกงานห้องปัก
                            </p>
                        </div>
