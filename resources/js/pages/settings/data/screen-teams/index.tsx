...
function stationLabel(station: 'screen' | 'flex'): string {
    return station === 'screen' ? 'สกรีน' : 'เฟล็ก';
}

export default function ScreenTeamsPage({ rows: initialRows }: PageProps) {
    const [rows, setRows] = useState<ScreenTeamRow[]>(initialRows);
    ...
    const filteredRows = useMemo(() => {
        ...
        return rows
            .filter((row) => {
                const matchesSearch =
                    keyword.length === 0 ||
                    row.team_name.toLowerCase().includes(keyword);
                const matchesStatus =
                    statusFilter === 'all' ||
                ...
                const matchesStation =
                    stationFilter === 'all' ||
                    row.station_name === stationFilter;

                return matchesSearch && matchesStatus && matchesStation;
            })
            .sort((a, b) => a.id - b.id);
    }, [rows, searchTerm, statusFilter, stationFilter]);
    ...
    const openEditModal = (row: ScreenTeamRow) => {
        setEditId(row.id);
        setForm({
    ...
    const saveForm = async () => {
        ...
        const endpoint =
            editId === null
                ? '/settings/data/screen-teams'
                : `/settings/data/screen-teams/${editId}`;
    ...
    const toggleActive = async (row: ScreenTeamRow) => {
        const response = await fetch(`/settings/data/screen-teams/${row.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken(),
    ...
    const deleteRow = async (row: ScreenTeamRow) => {
                    ...
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                        <div>
                            ...
                            <p className="mt-2 text-sm text-slate-600">
                                กำหนดทีมสกรีนและเฟล็กสำหรับหน้าแจกงาน
                            </p>
                        </div>
                        ...
                        <div className="flex w-full flex-col gap-3 md:w-auto md:flex-row">
                            ...
                            >
                                ...
                                <SelectContent>
                                    ...
                                    <SelectItem value="flex">เฟล็ก</SelectItem>
                                </SelectContent>
                            </Select>
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            กำหนดชื่อทีม ห้อง และสถานะการใช้งาน
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid gap-3">
                        ...
                        <div className="space-y-1">
                            ...
                            <Select
                                ...
                                onValueChange={(value: 'screen' | 'flex') =>
                                    setForm((prev) => ({
                                        ...prev,
                                        station_name: value,
                                    }))
...
ScreenTeamsPage.layout = () => ({
    breadcrumbs: [
        {
            title: 'จัดการทีมห้องสกรีน',
            href: '/settings/data/screen-teams',
        },
    ],
});
