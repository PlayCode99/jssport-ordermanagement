...
export default function SewingTeamsPage({ rows: initialRows }: PageProps) {
    const [rows, setRows] = useState<SewingTeamRow[]>(initialRows);
    ...
    const openEditModal = (row: SewingTeamRow) => {
        setEditId(row.id);
        setForm({
    ...
    const saveForm = async () => {
        ...
        const endpoint =
            editId === null
                ? '/settings/data/sewing-teams'
                : `/settings/data/sewing-teams/${editId}`;
    ...
    const toggleActive = async (row: SewingTeamRow) => {
        const response = await fetch(`/settings/data/sewing-teams/${row.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken(),
    ...
    const deleteRow = async (row: SewingTeamRow) => {
                    ...
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                        <div>
                            ...
                            <p className="mt-2 text-sm text-slate-600">
                                กำหนดทีมที่ใช้งานจริงในหน้าแจกงานห้องเย็บ
                            </p>
                        </div>
...
SewingTeamsPage.layout = () => ({
    breadcrumbs: [
        {
            title: 'จัดการทีมห้องเย็บ',
            href: '/settings/data/sewing-teams',
        },
    ],
});
